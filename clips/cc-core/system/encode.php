...
try {
   ...
   //writing phpinfo()
     ...
     $config->debugConversion ? App::log(CONVERSION_LOG, 'H.264 Encoding Command: ' . phpinfo()) : null; 
    ...
    $output = shell_exec($h264Command);
    $config->debugConversion ? App::log(CONVERSION_LOG, 'output shellcommand: ' . $output) : null;
    App::log($debugLog, $logMessage);
   ...
   //exec($h264Command);
    ...
    if (!file_exists($h264TempFilePath) || filesize($h264TempFilePath) < 1024*5) {
        ...
        //still copy file to H264 directory
    }
